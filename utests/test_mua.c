...
#define utest_mua_assert_find_str2(Mua, V) do{ \
    mua_item_type(x)* it = mua_find_pointed_zero_terminated(x, V); \
    utest_assert_clean(it); \
    utest_assert_clean(strcmp(*it, V) == 0); \
} while(0)
...
#define utest_mua_assert_find_not2(Mua, V) do{ \
    mua_item_type(x)* it = mua_find_pointed_zero_terminated(x, V); \
    utest_assert_clean(!it); \
} while(0)
...
int test_mua_find_string_pointed(void) {
    mua_char_ptr* x = &mua_ptr_create(char);
    ...
    utest_mua_assert_find_str2(x, foo1);
    utest_mua_assert_find_str2(x, "foo");
    utest_mua_assert_find_not2(x, "foobar");
    utest_mua_assert_find_str2(x, bar1);

    utest_mua_assert_find_not2(x, one);

    utest_mua_assert_find_not2(x, empty);
...
int main(void) {
    int failures = test_mua_0()
        ...
        + test_mua_string()
        + test_mua_find_int()
        + test_mua_find_string()
        + test_mua_find_string_pointed()
        ;
